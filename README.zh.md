...
- `/rp video`：基于角色上下文生成 AI 视频，可加 `--prompt` / `--style`（Gemini Veo 3.1）
...
- Telegram 自动媒体跟进：检测到用户旨意时自动生成图像、语音或视频
- 多模态请求自带限流（默认 5 秒窗口）

### 5. OpenClaw 原生集成

- 注册命令：`/rp`
- Hook 集成：`message_received`、`before_prompt_build`、`before_message_write`、`llm_output`
- 自动继承 OpenClaw 全局模型配置，支持 OpenAI-compatible / Gemini provider
- SQLite 默认持久化会话、资产、摘要与记忆向量
- **完全会话隔离**：RP 期间的消息通过 `before_message_write` 阻止写入主 agent 对话，`/rp end` 后主上下文完全干净
- 会话结束后自动注入 context break，强制 LLM 退出角色恢复原始人设
